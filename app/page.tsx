...
export default async function Home(){
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-950 to-black text-white">
      <NavBar />
      <main className="mx-auto max-w-6xl px-6 py-24 relative">
        ...
        <Reveal className="glass rounded-3xl p-12">
          <h1 className="text-5xl font-extrabold tracking-tight">{homeContent?.title ?? "Your modern portfolio"}</h1>
          <p className="mt-4 text-xl text-white/80">{homeContent?.summary ?? "Build a flexible portfolio with editable content."}</p>
          <p className="mt-6 text-white/70 max-w-12xl mx-auto">{homeContent?.body ?? "Use the admin panel to update your story, contact, and featured work."}</p>
          ...
          <Reveal className="glass-soft rounded-3xl p-8">
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="glass card-anim rounded-3xl p-5 text-left">
                ...
                <Counter target={projects.length} suffix="+" className="mt-2 block text-3xl font-bold" />
              </div>
              <div className="glass card-anim rounded-3xl p-5 text-left">
                <p className="text-sm uppercase tracking-[0.24em] text-white/60">{t.experience}</p>
                <p className="mt-2 block text-3xl font-bold" ><Counter target={5} suffix="+" /></p>
              </div>
              <div className="glass card-anim rounded-3xl p-5 text-left">
                <p className="text-sm uppercase tracking-[0.24em] text-white/60">{t.design}</p>
                <p className="mt-2 text-3xl font-bold">UI/UX + Web</p>
              </div>
            </div>
