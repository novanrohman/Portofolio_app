        ...
        <Reveal className="glass rounded-3xl p-8">
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-2">
            {portfolioItems.map((p) => (
              <article key={p.slug} className="rounded-2xl border border-white/6 p-4 bg-white/4 glass-img card-anim">
                <div className="flex items-start gap-4">
                  <div className="flex-1">
                    ...
                    {p.role && <p className="text-xs text-white/60">{[p.role, p.team].filter(Boolean).join(" · ")}</p>}
